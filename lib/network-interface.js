...
module.exports =  {
  ...
  async sendMiningSolutions()
    ...
    async collectMiningParameters( )
    {
      ...
      var result = await this.tokenContract.methods.getChallengeNumber().call();
      console.log('full result ' + result); // To see what's inside and decide how to extract the challengeNumber
      var challengeNumber = result[0]; // If it's an array or use the appropriate key if it's an object
  ...
  async submitMiningSolution(addressFrom,solution_number,challenge_digest){
   ...
   var mintMethod = this.tokenContract.methods.mint(solution_number,challenge_digest,0);
  ...
  try{
   ...
   var addressTo = this.tokenContract.options.address;
    ...
    var txData = this.web3.eth.abi.encodeFunctionCall({
        ...
        }, [solution_number, challenge_digest, 0]);

    var gweiToWei = 1e9;


    var gas_price_wei = this.gas_price_gwei * gweiToWei;
    var max_gas_cost = 1904624;
